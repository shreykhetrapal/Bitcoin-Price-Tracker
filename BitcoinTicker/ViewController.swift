...
//  BitcoinTicker
//
//  Created by Shrey Khetrapal on 20/08/2018.
//
...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    var currencySymbol = ["$", "R$", "$", "¥", "€", "£", "$", "Rp", "₪", "₹", "¥", "$", "kr", "$", "zł", "lei", "₽", "kr", "$", "$", "R"]
...
//    
    func getBTCprice(url: String) {
        ...
        Alamofire.request(url, method: .get).responseJSON { response in
                ...
                print("Success! Got the BTC Price")
                    let BTCprice : JSON = JSON(response.result.value!)

                    self.updateBTCprice(json: BTCprice)
